...
import {
  useCEODelegation,
  DELEGATION_MODULE_MAP,
  DELEGATION_APPROVAL_PATHS,
  DELEGATION_MODULE_TEMPLATES,
} from '../contexts/CEODelegationContext';
...
const Dashboard = () => {
  ...
  // CEO delegation state — supplied by CEODelegationContext
  const {
    isGlobalDelegate,
    delegatedTypes,    // [{ requestType, label, approvalPath, moduleKey, delegatedAt }]
  ...
  /**
   * Returns an array of module card definitions injected purely because the
   * user is a CEO delegate for one or more request types.
   *
   * Only produces cards for module keys that are NOT already rendered by the
   * user's own role (to avoid duplicates).
   */
  const buildDelegationOnlyModules = (existingModuleKeys) => {
    if (!delegatedTypes || delegatedTypes.length === 0) return [];

    // Group delegated types by module key so one card covers all types
    // in the same module (e.g. invoice + debit_note → both in 'invoices' card)
    const moduleGroups = {};
    delegatedTypes.forEach((d) => {
      const key = d.moduleKey || DELEGATION_MODULE_MAP[d.requestType];
      if (!key || existingModuleKeys.has(key)) return; // already visible
      if (!moduleGroups[key]) moduleGroups[key] = [];
      moduleGroups[key].push(d);
    });

    return Object.entries(moduleGroups).map(([moduleKey, types]) => {
      const template = DELEGATION_MODULE_TEMPLATES[moduleKey];
      if (!template) return null;

      // Build one action button per delegated type in this module
      const actions = types.map((d) => ({
        label:        `Delegated CEO: ${d.label}`,
        path:         d.approvalPath || DELEGATION_APPROVAL_PATHS[d.requestType] || '/dashboard',
        icon:         <CrownOutlined />,
        primary:      true,
        isDelegation: true,
      }));
      ...
      return {
        ...
        managementRoles:  [],
        isDelegationOnly: true,
        // Delegation cards bypass the normal actions lookup — provide a
        // synthetic role key matching the current user's role so the
        // action-building logic inside getModuleCards finds the actions.
        actions: {
          [user?.role]: actions,
          ceo:          actions,
          base:         [],
  ...
  const getModuleCards = () => {
    ...
    const modules = [
      ...
      // ── 8. INCIDENT REPORTS ───────────────────────────────────────────────
      {
        ...
        actions: {
          base: [
            ...
            { label: 'Incident Report', path: '/employee/site-incident', icon: <ArrowRightOutlined /> }
          ],
          supervisor: [
          ...
          technical: [
            { label: 'Department View', path: '/supervisor/incident-reports', icon: <TeamOutlined /> },
            { label: 'Department View Site', path: '/supervisor/engineering-incident-reports', icon: <TeamOutlined /> }
          ],
          hse: [
...

      {
        ...
        description: 'Send PDFs for signature with a custom or hierarchical approval chain',
        icon: <FilePdfOutlined style={{ fontSize: '48px', color: '#f5222d' }} />,
        color: '#fff1f0',
        borderColor: '#f5222d',
        stats: { pending: 0, total: 0 },
        managementRoles: ['admin', 'it', 'ceo'],
        actions: {
          base: [
            { label: 'My Documents', path: '/employee/documents/sign', icon: <UserOutlined /> },
            { label: 'New Document', path: '/employee/documents/sign/new', icon: <ArrowRightOutlined /> }
          ]
        }
      }
    ];
    ...
    // Collect the keys of modules the user normally sees
    const existingModuleKeys = new Set(modules.map((m) => m.key));

    // Build extra cards for delegated modules not already visible
    const delegationOnlyModules = buildDelegationOnlyModules(existingModuleKeys);

    // Merge delegation cards into the full list
    const allModules = [...modules, ...delegationOnlyModules];

    // For delegated types whose module IS already visible, note which extra
    // delegation action buttons should be injected at the top of that card.
    // Shape: { [moduleKey]: [ actionObject, ... ] }
    const injectedDelegationActions = {};
    (delegatedTypes || []).forEach((d) => {
      const key = d.moduleKey || DELEGATION_MODULE_MAP[d.requestType];
      if (!key || !existingModuleKeys.has(key)) return; // handled by delegation-only card
      if (!injectedDelegationActions[key]) injectedDelegationActions[key] = [];
      injectedDelegationActions[key].push({
        label:        `🏛️ Delegated CEO: ${d.label}`,
        path:         d.approvalPath || DELEGATION_APPROVAL_PATHS[d.requestType] || '/dashboard',
    ...
    return allModules.map(module => {
      ...
      // ── Prepend delegation action buttons for already-visible modules ──
      const extraDelegation = injectedDelegationActions[module.key] || [];
      const allFinalActions = [
        ...extraDelegation,
        ...finalActions.filter(
          // avoid duplicating if the delegation path is already in normal actions
          (a) => !extraDelegation.some((d) => d.path === a.path)
        ),
      ];
      // ── END ──

      const visibleActions = allFinalActions.slice(0, 3);
      const expandableActions = allFinalActions.slice(3);
      const isExpanded = expandedCards[module.key];

      const renderActionButton = (action, index, isExpandable = false) => {
        // ── NEW: delegation actions get a gold outline style ──────────────
        if (action.isDelegation) {
          return (
            <Button
              key={`delegation-${action.path}-${index}`}
              block
              icon={action.icon || <CrownOutlined />}
        ...
        // ── Original renderActionButton logic (unchanged) ─────────────────
      ...
      // ── ICON RESOLUTION for delegation-only cards ──────────────────────
      // Delegation-only modules carry `iconType` (string) instead of JSX.
      // Resolve it here so the Card render can use `iconJSX` uniformly.
      const iconJSX = module.icon || (() => {
        const iconMap = {
          DollarOutlined:       <DollarOutlined       style={{ fontSize: '48px', color: module.iconColor }} />,
          ShoppingCartOutlined: <ShoppingCartOutlined style={{ fontSize: '48px', color: module.iconColor }} />,
          FileTextOutlined:     <FileTextOutlined     style={{ fontSize: '48px', color: module.iconColor }} />,
          SolutionOutlined:     <SolutionOutlined     style={{ fontSize: '48px', color: module.iconColor }} />,
          BankOutlined:         <BankOutlined         style={{ fontSize: '48px', color: module.iconColor }} />,
          WalletOutlined:       <WalletOutlined       style={{ fontSize: '48px', color: module.iconColor }} />,
          ContactsOutlined:     <ContactsOutlined     style={{ fontSize: '48px', color: module.iconColor }} />,
          ScheduleOutlined:     <ScheduleOutlined     style={{ fontSize: '48px', color: module.iconColor }} />,
        };
        return iconMap[module.iconType] || <CrownOutlined style={{ fontSize: '48px', color: '#faad14' }} />;
      })();
      // ── END ICON RESOLUTION ──
        ...
        <Col xs={24} sm={24} md={12} lg={8} xl={8} key={module.key}>
          <Card
            ...
            style={{
              ...
              borderRadius: '12px',
              position: 'relative',
              // ── gold dashed border for delegation-only cards ──
              ...(module.isDelegationOnly && {
          ...
          >
            ...
            {/* ── CEO Delegation badge for delegation-only cards ── */}
            {module.isDelegationOnly && (
              <div
                style={{
                  position: 'absolute',
            ...
            <div style={{ textAlign: 'center', marginBottom: '20px', marginTop: module.isDelegationOnly ? '20px' : '0' }}>
              {iconJSX}
              ...
              <Row gutter={[16, 16]} style={{ marginBottom: '20px' }}>
                <Col span={24}>
                  <Alert
                    ...
                    showIcon
                    icon={<CrownOutlined />}
                    message="CEO approval authority active"
                    description="You are acting on Mr. Tom's behalf for this module."
                    style={{ fontSize: '12px' }}
                  />
                </Col>
              </Row>
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      {/* ── GLOBAL CEO DELEGATION BANNER ──────────────────────────────────
          Shown only when the current user is Tom's global (all-types) delegate
      ─────────────────────────────────────────────────────────────────────── */}
      {isGlobalDelegate && (
        <Alert
          type="warning"
          showIcon
          icon={<CrownOutlined />}
          style={{ marginBottom: '24px', borderRadius: '12px', border: '2px solid #faad14' }}
          message={
            <Text strong style={{ color: '#d48806' }}>
              You are acting as Global CEO Delegate
            </Text>
          }
          description={
            <Space direction="vertical" size="small">
              <Text>
                Mr. Tom is currently unavailable. All CEO-level final approvals are
                being routed to you. Use the <Text strong>Final Approval Queue</Text> to
                act on any pending requests.
              </Text>
              <Button
                type="primary"
                icon={<CrownOutlined />}
                onClick={() => navigate('/admin/head-approval')}
                style={{ backgroundColor: '#faad14', borderColor: '#faad14', marginTop: '8px' }}
              >
                Open Final Approval Queue
              </Button>
            </Space>
          }
        />
      ...
      {/* ── PER-TYPE DELEGATION SUMMARY BANNER ───────────────────────────
          Shown when the user has per-type (but not global) delegations.
          Links directly to each delegated approval queue.
      ─────────────────────────────────────────────────────────────────────── */}
      {!isGlobalDelegate && delegatedTypes.length > 0 && (
        <Alert
          type="info"
          showIcon
          icon={<SwapOutlined />}
          ...
          message={
            <Text strong>
              You have CEO delegated approval authority for{' '}
              {delegatedTypes.length} request type{delegatedTypes.length !== 1 ? 's' : ''}
            </Text>
          }
          description={
            <Space wrap style={{ marginTop: '8px' }}>
              {delegatedTypes.map((d) => (
                <Button
                  key={d.requestType}
                  size="small"
                  icon={<CrownOutlined />}
                  ...
                  onClick={() => navigate(d.approvalPath || '/dashboard')}
                >
                  {d.label}
                </Button>
              ))}
...
//         color: '#fff1f0',
//         borderColor: '#f5222d',
//         stats: { pending: 0, total: 0 }, // wire to a /document-signing dashboard-stats endpoint if desired
//         managementRoles: ['admin', 'it', 'ceo'],
//         actions: {
//           base: [
//             { label: 'My Documents', path: '/employee/documents/sign', icon: <UserOutlined /> },
