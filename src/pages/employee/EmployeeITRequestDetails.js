...
import { EmployeeAcknowledgmentCard } from './EmployeeAcknowledgmentCard';
...
const EmployeeITRequestDetails = () => {
  ...
  const getStatusTag = (status) => {
    const statusMap = {
      ...
      'pending_acknowledgment': { color: 'gold', icon: <ClockCircleOutlined />, text: 'Awaiting Your Acknowledgment' },
      'discharge_complete': { color: 'green', icon: <CheckCircleOutlined />, text: 'Discharge Complete' },
    };
  ...
  const canEdit = () => request?.status === 'draft' && request?.employee?._id === user.userId;
  const canDelete = () => request?.status === 'draft' && request?.employee?._id === user.userId;

  // Show acknowledgment section
  const showAcknowledgment =
    request?.requestType === 'material_request' &&
    ['pending_acknowledgment', 'discharge_complete'].includes(request?.status);
    ...
    return (
      <div style={{ padding: '24px' }}>
        <Alert
          ...
          type="error"
          showIcon
          action={<Button onClick={fetchRequestDetails}>Retry</Button>}
        />
      </div>
    );
  }
    ...
    <div style={{ padding: '24px' }}>
      ...
      <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
          ...
          <Space>
            <Button icon={<ReloadOutlined />} onClick={fetchRequestDetails} loading={loading}>
              Refresh
            </Button>
            {canEdit() && (
              <Button type="default" icon={<EditOutlined />} onClick={() => navigate(`/employee/it-support/edit/${request._id}`)}>
                Edit
              </Button>
            )}
            {canDelete() && (
      ...
      {request.status === 'pending_acknowledgment' && (
        <Alert
          message="Action Required — Please Acknowledge Receipt"
          description="IT has dispatched items for your request. Please scroll down to review and confirm receipt."
          type="warning"
          showIcon
      ...
      {request.status === 'resolved' && (
        <Alert
          ...
          description="Your IT request has been successfully resolved."
          type="success"
          showIcon
          style={{ marginBottom: '24px' }}
        />
      ...
      {request.status === 'discharge_complete' && (
        <Alert
          ...
          description="You have acknowledged receipt of the dispatched items. Download the discharge PDF for your records."
          type="success"
          showIcon
          style={{ marginBottom: '24px' }}
        />
        ...
        <Col xs={24} lg={16}>
          ...
          <Card title="Request Information" style={{ marginBottom: '16px' }}>
            <Descriptions column={1} bordered>
              ...
              <Descriptions.Item label="Priority">{getPriorityTag(request.priority)}</Descriptions.Item>
              <Descriptions.Item label="Type">
                {request.requestType === 'material_request' ? '🛒 Material Request' : '🔧 Technical Issue'}
              </Descriptions.Item>
              <Descriptions.Item label="Category"><Tag>{request.category}</Tag></Descriptions.Item>
              <Descriptions.Item label="Subcategory">{request.subcategory || 'N/A'}</Descriptions.Item>
              <Descriptions.Item label="Title"><Text strong>{request.title}</Text></Descriptions.Item>
              <Descriptions.Item label="Description"><Paragraph>{request.description}</Paragraph></Descriptions.Item>
              ...
              <Descriptions.Item label="Location">{request.location || 'N/A'}</Descriptions.Item>
            ...
            <Card title="Requested Items" style={{ marginBottom: '16px' }}>
              {request.requestedItems.map((item, index) => (
                <Card key={index} size="small" style={{ marginBottom: '8px' }}>
                  <Row gutter={16}>
                    <Col span={12}><Text strong>Item:</Text> {item.item}</Col>
                    <Col span={6}><Text strong>Quantity:</Text> {item.quantity}</Col>
                    <Col span={6}><Text strong>Est. Cost:</Text> {item.estimatedCost?.toLocaleString()} XAF</Col>
                  </Row>
                  {item.brand && (
          ...
          {request.requestType === 'technical_issue' && request.deviceDetails && (
            <Card title="Device Information" style={{ marginBottom: '16px' }}>
              <Descriptions column={2} bordered>
                <Descriptions.Item label="Device Type">{request.deviceDetails.deviceType || 'N/A'}</Descriptions.Item>
                <Descriptions.Item label="Brand">{request.deviceDetails.brand || 'N/A'}</Descriptions.Item>
                <Descriptions.Item label="Model">{request.deviceDetails.model || 'N/A'}</Descriptions.Item>
                <Descriptions.Item label="Serial Number">{request.deviceDetails.serialNumber || 'N/A'}</Descriptions.Item>
                <Descriptions.Item label="Operating System" span={2}>{request.deviceDetails.operatingSystem || 'N/A'}</Descriptions.Item>
              </Descriptions>
            </Card>
          )}
            ...
            <Card title="Issue Details" style={{ marginBottom: '16px' }}>
              <Descriptions column={2} bordered>
                ...
                {request.issueDetails.frequency && (
                  <Descriptions.Item label="Frequency">{request.issueDetails.frequency}</Descriptions.Item>
                )}
          ...
          {request.attachments && request.attachments.length > 0 && (
            <Card
              title={<Space><FileImageOutlined />Attachments ({request.attachments.length})</Space>}
              style={{ marginBottom: '16px' }}
            >
              <Row gutter={[16, 16]}>
                    ...
                    <Col xs={24} sm={12} md={8} key={index}>
                      <Card
                        ...
                        cover={
                          isImage ? (
                            <div
                              style={{ height: '150px', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#f5f5f5', cursor: 'pointer' }}
                            ...
                            <div style={{ height: '150px', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#fafafa' }}>
                        ...
                        actions={[
                          isImage && (
                            <Button type="link" icon={<FileImageOutlined />} onClick={() => handlePreview(attachment)}>
                              Preview
                            </Button>
                          ),
                      ...
                      >
                        <Card.Meta
                          title={<Text ellipsis={{ tooltip: attachment.name }} style={{ fontSize: '13px' }}>{attachment.name}</Text>}
                          description={<Text type="secondary" style={{ fontSize: '11px' }}>{(attachment.size / 1024).toFixed(2)} KB</Text>}
                        />
                      </Card>
                    </Col>
                  );
          ...
          {request.resolution && request.resolution.description && (
            <Card title="Resolution" style={{ marginBottom: '16px' }}>
              <Descriptions column={1} bordered>
                <Descriptions.Item label="Resolved By">{request.resolution.resolvedBy}</Descriptions.Item>
                      ...
                      description={
                        <div>
                          ...
                          {step.status !== 'pending' && (
                            <>
                              <div style={{ marginTop: '4px' }}>
                                <Tag color={step.status === 'approved' ? 'green' : 'red'} size="small">{step.status}</Tag>
                              </div>
                              {step.actionDate && (
                              ...
                              {step.comments && (
                                <div style={{ marginTop: '4px', fontSize: '12px', padding: '4px 8px', backgroundColor: '#f5f5f5', borderRadius: '4px' }}>
                                  "{step.comments}"
                                </div>
                              )}
                            </>
            ...
            <Timeline>
              ...
              {request.approvalChain?.filter(step => step.status !== 'pending').map((step, index) => (
                ...
                >
                  ...
                  {step.comments && (
                    <div style={{ marginTop: '4px', padding: '8px', backgroundColor: '#f5f5f5', borderRadius: '4px', fontSize: '12px' }}>
                      "{step.comments}"
                    </div>
                  )}
                </Timeline.Item>
              ...
              {['it_assigned', 'in_progress', 'resolved', 'pending_acknowledgment', 'discharge_complete'].includes(request.status) && (
                <>
                  ...
                  {['resolved', 'pending_acknowledgment', 'discharge_complete'].includes(request.status) && (
                  ...
                  {['pending_acknowledgment', 'discharge_complete'].includes(request.status) && (
                    <Timeline.Item color="orange" dot={<ClockCircleOutlined />}>
                      <Text strong>Items Dispatched by IT</Text>
                      <br />
                      <Text type="secondary" style={{ fontSize: '12px' }}>
                  ...
                  {request.status === 'discharge_complete' && (
                    <Timeline.Item color="green" dot={<CheckCircleOutlined />}>
                      <Text strong>You Acknowledged Receipt</Text>
                      <br />
                      <Text type="secondary" style={{ fontSize: '12px' }}>
                        {request.acknowledgmentSignature?.signedAt
                          ? dayjs(request.acknowledgmentSignature.signedAt).format('MMM DD, YYYY HH:mm')
                          : '—'}
                      </Text>
                    </Timeline.Item>
                  )}
