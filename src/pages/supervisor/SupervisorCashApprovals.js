...
const PDFViewer = ({ url, name, authHeaders }) => {
  ...
  if (loading) return (
    <div style={{ display:'flex', justifyContent:'center', alignItems:'center', height:'70vh', border:'1px solid #d9d9d9', borderRadius:'6px' }}>
      <Spin size="large" /><span style={{ marginLeft:'16px' }}>Loading PDF...</span>
    </div>
  );
  if (error) return (
...
// ── Status config covering EVERY CashRequest status value ────────────────────
const CASH_STATUS_MAP = {
  ...
  pending_finance:                 { color: 'blue',    text: 'Pending Finance',             icon: <BankOutlined /> },
  pending_head_of_business:        { color: 'purple',  text: 'Pending Head of Business',   icon: <CrownOutlined /> },
  pending_ceo:                     { color: 'magenta', text: 'Pending CEO',                icon: <CrownOutlined /> },
  ...
  partially_disbursed:             { color: 'geekblue',text: 'Partially Disbursed',        icon: <DollarOutlined /> },
  ...
  justification_pending_head_of_business: { color: 'purple',  text: 'Justification — Head of Biz',   icon: <CrownOutlined /> },
  ...
  justification_rejected_head_of_business: { color: 'gold',    text: 'Just. Rejected — Head of Biz',  icon: <ExclamationCircleOutlined /> },
  justification_rejected_ceo:              { color: 'gold',    text: 'Just. Rejected — CEO',          icon: <ExclamationCircleOutlined /> },
};

// Statuses that belong to the justification workflow
const JUSTIFICATION_STATUSES = new Set([
...
// Statuses considered "terminal" / no longer pending initial approval
const TERMINAL_STATUSES = new Set([
  'approved', 'denied', 'rejected',
  'disbursed', 'partially_disbursed', 'fully_disbursed', 'completed',
  ...JUSTIFICATION_STATUSES
]);

// Map justification status → numeric level for display
...
// ── Helper: get the level from a justification status ────────────────────────
const extractJustificationLevel = (status) =>
  JUSTIFICATION_LEVEL_MAP[status] ?? null;

// ── Helper: is this a justification-phase status? ────────────────────────────
const isJustificationStatus = (status) => JUSTIFICATION_STATUSES.has(status);

// ── Helper: derive the "actual" workflow status from the approval chain ───────
// The DB status field may lag; reading the chain directly is more reliable.
const getActualStatusFromChain = (request) => {
  if (!request?.approvalChain?.length) return request?.status;
  // Find the first pending step — that tells us whose turn it is
  const pending = request.approvalChain.find(s => s.status === 'pending');
  if (!pending) return request.status; // no pending step → use DB value
  // Map the approver's role to the matching status string
  const roleStatusMap = {
  ...
  return roleStatusMap[pending.approver?.role] ?? request.status;
};

// ── Helper: disbursement summary (handles both V1 and V2 schemas) ─────────────
const getDisbursementSummary = (req) => ({
  totalDisbursed:   req.totalDisbursed   ?? req.disbursementDetails?.amount ?? 0,
  remainingBalance: req.remainingBalance ?? 0,
  amountApproved:   req.amountApproved   ?? req.amountRequested ?? 0,
  disbursements:    req.disbursements    ?? (req.disbursementDetails ? [req.disbursementDetails] : []),
});

// ── Reusable itemized-breakdown table ────────────────────────────────────────
const ItemizedTable = ({ data }) => {
      ...
      <Table
        ...
        columns={[
          ...
          { title: 'Category', dataIndex: 'category', key: 'category',
            render: c => c ? <Tag>{c.replace(/-/g,' ').toUpperCase()}</Tag> : '—' },
          { title: 'Amount', dataIndex: 'amount', key: 'amount',
            render: a => <Text strong>XAF {Number(a||0).toLocaleString()}</Text> }
        ]}
        summary={rows => {
          ...
          return (
            <Table.Summary.Row>
              ...
              <Table.Summary.Cell index={1}>
                <Text strong style={{ color:'#1890ff' }}>XAF {total.toLocaleString()}</Text>
              </Table.Summary.Cell>
            </Table.Summary.Row>
          );
        }}
...
const ApprovalTimeline = ({ chain, currentStatus, userEmail }) => {
  if (!chain?.length) return <Text type="secondary">No approval chain available</Text>;
  const currentLevel = isJustificationStatus(currentStatus)
    ? extractJustificationLevel(currentStatus)
  ...
  return (
    <Timeline>
      {chain.map((step, i) => {
        ...
        const dot   = step.status === 'approved' ? <CheckCircleOutlined />
                    : step.status === 'rejected' ? <CloseCircleOutlined />
                    : <ClockCircleOutlined />;
        const isCurrentLevel = step.level === currentLevel;
        const isUserStep     = step.approver?.email?.toLowerCase() === userEmail?.toLowerCase();
        return (
          <Timeline.Item key={i} color={color} dot={dot}>
            ...
            <br />
            {step.status === 'pending' && (
              <Tag color={isCurrentLevel ? 'gold' : 'orange'}>{isCurrentLevel ? 'Awaiting Action' : 'Pending'}</Tag>
            )}
            {step.status === 'approved' && (
              <>
                <Tag color="green">Approved</Tag>
                {step.actionDate && <Text type="secondary"> on {new Date(step.actionDate).toLocaleDateString('en-GB')}{step.actionTime ? ` at ${step.actionTime}` : ''}</Text>}
            ...
            {step.status === 'rejected' && (
              <>
                ...
                {step.actionDate && <Text type="secondary"> on {new Date(step.actionDate).toLocaleDateString('en-GB')}{step.actionTime ? ` at ${step.actionTime}` : ''}</Text>}
                {step.comments && <div style={{ marginTop:4, color:'#ff4d4f' }}><Text>Reason: "{step.comments}"</Text></div>}
              </>
            )}
          </Timeline.Item>
        );
...
const SupervisorCashApprovals = () => {
  ...
  const [justDecisionModalVisible,setJustDecisionModalVisible] = useState(false);
  const [selectedRequest,       setSelectedRequest]       = useState(null);
  const [selectedJustification, setSelectedJustification] = useState(null);
  const [activeTab,   setActiveTab]   = useState('pending');
  const [stats,       setStats]       = useState({ pending:0, approved:0, rejected:0, total:0, justificationsPending:0 });
  ...
  const hasUserApproved = useCallback((request) => {
    if (!request?.approvalChain?.length || !user?.email) return false;
    if (request.teamRequestMetadata) return request.teamRequestMetadata.userHasApproved === true;
    return request.approvalChain.some(s =>
      s.approver?.email?.toLowerCase() === user.email.toLowerCase() && s.status === 'approved'
    );
  }, [user?.email]);

  const hasUserRejected = useCallback((request) => {
    if (!request?.approvalChain?.length || !user?.email) return false;
    if (request.teamRequestMetadata) return request.teamRequestMetadata.userHasRejected === true;
    return request.approvalChain.some(s =>
      s.approver?.email?.toLowerCase() === user.email.toLowerCase() && s.status === 'rejected'
    );
  }, [user?.email]);
  ...
  const getStatusTag = (status, request = null) => {
    // For initial-approval requests, derive status from chain when possible
    const displayStatus = (request && !isJustificationStatus(status))
      ? getActualStatusFromChain(request)
      : status;

    const info = CASH_STATUS_MAP[displayStatus]
      || { color:'default', text: (displayStatus||'Unknown').replace(/_/g,' '), icon: null };

    // Justification statuses get a tooltip with level description
    if (isJustificationStatus(displayStatus)) {
      const level    = extractJustificationLevel(displayStatus);
      const roleName = LEVEL_ROLE_NAMES[level] || `Level ${level}`;
      const desc     = displayStatus.includes('rejected')
        ? `Revision required — returned to ${roleName}`
        : `Awaiting approval from ${roleName} (Level ${level})`;
      return (
        <Tooltip title={desc}>
  ...
  const getUrgencyTag = (urgency) => {
    const map = { high:{ color:'red', text:'High' }, medium:{ color:'orange', text:'Medium' }, low:{ color:'green', text:'Low' } };
    const u   = (urgency || '').toLowerCase();
    const cfg = map[u] || { color:'default', text: urgency || '—' };
    return <Tag color={cfg.color}>{cfg.text}</Tag>;
  };

  const getRequestModeTag = (mode) =>
    mode === 'reimbursement'
      ? <Tag color="purple" icon={<DollarOutlined />}>Reimbursement</Tag>
      : <Tag color="blue"   icon={<FileTextOutlined />}>Cash Advance</Tag>;

  const getApprovalProgress = (request) => {
    if (!request?.approvalChain?.length) return 0;
  ...
  const getFilteredRequests = () => {
    const cashOnly = requests.filter(r => !isJustificationStatus(r.status));
    switch (activeTab) {
      case 'pending':  return cashOnly.filter(r => canUserApprove(r));
      case 'approved': return cashOnly.filter(r => hasUserApproved(r));
      case 'rejected': return cashOnly.filter(r => hasUserRejected(r));
      default:         return cashOnly;
    }
  };
  ...
  const fetchCashRequests = useCallback(async () => {
    try {
      ...
      if (response.success) {
        ...
        const cashOnly  = data.filter(r => !isJustificationStatus(r.status));
        setRequests(data);
        setStats(prev => ({
          ...prev,
          pending:  cashOnly.filter(r => canUserApprove(r)).length,
          approved: cashOnly.filter(r => hasUserApproved(r)).length,
          rejected: cashOnly.filter(r => hasUserRejected(r)).length,
          total:    cashOnly.length,
        }));
      } else {
  ...
  const fetchJustifications = useCallback(async () => {
    try {
      ...
      if (response.success) {
        const data   = (response.data || []).filter(j => isJustificationStatus(j.status));
        setJustifications(data);
        setStats(prev => ({
          ...prev,
          justificationsPending: data.filter(j => canUserApproveJustification(j)).length,
        }));
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (user?.email) { fetchCashRequests(); fetchJustifications(); }
  }, [fetchCashRequests, fetchJustifications, user?.email]);

  // Auto-open approval modal from email link
  useEffect(() => {
    const id = autoApprovalId || autoRejectId;
    if (!id) return;
    (async () => {
      try {
        const resp = await cashRequestAPI.getRequestById(id);
        if (resp.success) {
          setSelectedRequest(resp.data);
          setApprovalModalVisible(true);
          form.setFieldsValue({ decision: autoApprovalId ? 'approved' : 'rejected' });
        }
      } catch { message.error('Failed to load cash request for approval'); }
    })();
  ...
  const handleApprovalDecision = async (values) => {
    ...
    try {
      setLoading(true);
      const response = await cashRequestAPI.processSupervisorDecision(selectedRequest._id, {
        decision: values.decision, comments: values.comments
      });
      if (response.success) {
        const verb = values.decision === 'approved' ? 'approved' : 'rejected';
        message.success(`Cash request ${verb} successfully`);
        ...
        notification.success({
          ...
          description: `Request from ${selectedRequest.employee?.fullName} has been ${verb}.`,
          duration: 4
        });
  ...
  const handleJustificationDecision = async (values) => {
    ...
    try {
      setLoading(true);
      const response = await cashRequestAPI.processJustificationDecision(selectedJustification._id, {
        decision: values.decision, comments: values.comments
      });
      if (response.success) {
        const verb = values.decision === 'approved' ? 'approved' : 'rejected';
        message.success(`Justification ${verb} successfully`);
        setJustDecisionModalVisible(false);
        form.resetFields();
        setSelectedJustification(null);
        await Promise.all([fetchCashRequests(), fetchJustifications()]);
        notification.success({
          message: 'Decision Recorded',
          description: `Justification for ${selectedJustification.employee?.fullName} has been ${verb}.`,
          duration: 4
        });
      } else throw new Error(response.message);
    } catch (error) {
  ...
  const handleViewDetails = async (request) => {
    try {
      const resp = await cashRequestAPI.getRequestById(request._id);
      if (resp.success) { setSelectedRequest(resp.data); setDetailsModalVisible(true); }
    } catch { message.error('Failed to fetch request details'); }
  };

  const handleViewJustificationDetails = async (record) => {
    try {
      const resp = await cashRequestAPI.getRequestById(record._id);
      if (resp.success) { setSelectedJustification(resp.data); setDetailsModalVisible(true); }
    } catch { message.error('Failed to fetch justification details'); }
  };

  const handleDownloadAttachment = async (attachment) => {
    if (!attachment?.publicId) { message.error('Invalid attachment — no publicId'); return; }
    ...
    try {
      ...
      a.href = url; a.download = attachment.name || attachment.publicId;
      document.body.appendChild(a); a.click(); document.body.removeChild(a);
      window.URL.revokeObjectURL(url);
      hide(); message.success(`Downloaded ${attachment.name || 'file'}`);
    } catch (e) { hide(); message.error(`Failed to download: ${e.message}`); }
  };

  const handleViewAttachment = async (attachment) => {
    if (!attachment) { message.error('No attachment data'); return; }
    setFileViewerLoading(true);
    setFileViewerVisible(true);
    try {
      const token  = localStorage.getItem('token');
      const apiUrl = process.env.REACT_APP_API_URL || 'http://localhost:5001/api';
      const pid    = attachment.publicId || attachment.fileName || attachment.name;
      if (!pid) throw new Error('No public ID');
      ...
      if (isPDF) {
        setViewingFile({ name: attachment.name, url: `${apiUrl}/files/view/${encodeURIComponent(pid)}`,
          type:'pdf', authHeaders:{ Authorization:`Bearer ${token}` }, isDirectUrl: true });
      } else {
        const resp = await fetch(`${apiUrl}/files/download/${encodeURIComponent(pid)}`, {
          headers: { Authorization:`Bearer ${token}` }
        });
        if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
        const blob = await resp.blob();
        setViewingFile({ name: attachment.name, url: window.URL.createObjectURL(blob),
          type: isImage ? 'image' : 'other', mimetype: attachment.mimetype });
      }
    } catch (e) {
  ...
  const closeFileViewer = () => {
    if (viewingFile?.url && !viewingFile?.isDirectUrl) window.URL.revokeObjectURL(viewingFile.url);
    setFileViewerVisible(false);
    setViewingFile(null);
  ...
  const columns = [
    {
      ...
      render: (_, r) => (
        <div>
          ...
          <div style={{ marginTop:'4px' }}>{getRequestModeTag(r.requestMode)}</div>
        </div>
      )
    },
    {
      ...
      render: (_, r) => (
        <div>
          {r.requestMode === 'reimbursement' ? (
            <>
              <Tag color="purple" size="small">💰 REIMBURSEMENT</Tag><br />
              <Text type="secondary" style={{ fontSize:'12px' }}>Advance: XAF {(r.advanceReceived||0).toLocaleString()}</Text><br />
              <Text type="secondary" style={{ fontSize:'12px' }}>Spent: XAF {(r.amountSpent||0).toLocaleString()}</Text><br />
              <Text strong>Due: XAF {((r.amountSpent||0)-(r.advanceReceived||0)).toLocaleString()}</Text>
            </>
          ) : (
            <>
              <Tag color="blue" size="small">📝 CASH ADVANCE</Tag><br />
              <Text strong>XAF {(r.amountRequested||0).toLocaleString()}</Text>
            </>
          )}
          <br />
          ...
          {r.itemizedBreakdown?.length > 0 && <Tag color="cyan" size="small">{r.itemizedBreakdown.length} items</Tag>}
          {r.reimbursementDetails?.receiptDocuments?.length > 0 && (
            <Tag color="green" size="small" icon={<CheckCircleOutlined />}>
              {r.reimbursementDetails.receiptDocuments.length} receipt(s)
            </Tag>
          )}
          <br />
      ...
      render: (_, r) => (
        <div>
          ...
          <Text type="secondary" style={{ fontSize:'11px' }}>
            <CalendarOutlined /> Required: {r.requiredDate ? new Date(r.requiredDate).toLocaleDateString('en-GB') : 'N/A'}
          </Text><br />
          <Text type="secondary" style={{ fontSize:'11px' }}>
    ...
    },
    {
      ...
      render: (_, r) => (
        <div>
          {getStatusTag(r.status, r)}
          {canUserApprove(r) && <div style={{ marginTop:4 }}><Tag color="gold" size="small">Your Turn</Tag></div>}
        </div>
      )
    },
    {
      ...
      width: 100,
      render: (_, r) => {
        const pct    = getApprovalProgress(r);
        const status = r.status === 'denied' ? 'exception'
          : ['approved','disbursed','fully_disbursed','completed'].includes(r.status) ? 'success' : 'active';
        return <div style={{ width:80 }}>
          <Progress percent={pct} size="small" status={status} showInfo={false} />
          <Text style={{ fontSize:'11px' }}>{pct}%</Text>
        </div>;
      }
    },
    {
      ...
      render: (_, r) => (
        <Space size="small">
          <Button size="small" icon={<EyeOutlined />} onClick={() => handleViewDetails(r)}>View</Button>
          {canUserApprove(r) && (
            <Button size="small" type="primary" onClick={() => {
              setSelectedRequest(r); form.resetFields(); setApprovalModalVisible(true);
            }}>Review</Button>
          )}
        </Space>
      )
  ...
  const justificationColumns = [
    ...
    },
    {
      ...
      render: (_, r) => (
        <div>
          <Text strong>REQ-{r._id?.slice(-6).toUpperCase()}</Text><br />
          <Text type="secondary" style={{ fontSize:'12px' }}>
            {r.requestType?.replace(/-/g,' ').toUpperCase()}
          </Text>
        </div>
      )
    },
      ...
      render: (_, r) => (
        <Space size="small">
          <Button size="small" icon={<EyeOutlined />}
            onClick={() => handleViewJustificationDetails(r)}>View</Button>
          {canUserApproveJustification(r) && (
            <Button size="small" type="primary" icon={<AuditOutlined />}
              onClick={() => { setSelectedJustification(r); form.resetFields(); setJustDecisionModalVisible(true); }}>
              Review
            </Button>
          )}
        </Space>
  ...
  // ── Decision form (shared between cash and justification modals) ─────────────
  const DecisionForm = ({ onFinish, onCancel, submitLabel = 'Submit Decision' }) => (
    <Form form={form} layout="vertical" onFinish={onFinish}>
      <Form.Item name="decision" label="Your Decision" rules={[{ required:true, message:'Please make a decision' }]}>
        <Radio.Group>
          <Radio.Button value="approved"><CheckCircleOutlined style={{ color:'#52c41a' }} /> Approve</Radio.Button>
          <Radio.Button value="rejected"><CloseCircleOutlined style={{ color:'#f5222d' }} /> Reject</Radio.Button>
        </Radio.Group>
      </Form.Item>
      <Form.Item name="comments" label="Comments" rules={[{ required:true, message:'Comments are required for audit trail' }]}>
        <TextArea rows={4} placeholder="Explain your decision…" showCount maxLength={500} />
      </Form.Item>
      <Form.Item>
        <Space>
  ...
  return (
    <div style={{ padding:'24px' }}>
      <Card>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'24px' }}>
          ...
          <Button icon={<ReloadOutlined />} loading={loading}
            onClick={() => Promise.all([fetchCashRequests(), fetchJustifications()]).then(() => message.success('Refreshed'))}>
            Refresh
          </Button>
        </div>

        <Row gutter={16} style={{ marginBottom:'24px' }}>
          <Col span={5}><Statistic title="Pending Your Approval"  value={stats.pending}               prefix={<ClockCircleOutlined />}  valueStyle={{ color:'#faad14' }} /></Col>
          <Col span={5}><Statistic title="Approved by You"        value={stats.approved}              prefix={<CheckCircleOutlined />}  valueStyle={{ color:'#52c41a' }} /></Col>
          <Col span={5}><Statistic title="Rejected by You"        value={stats.rejected}              prefix={<CloseCircleOutlined />}  valueStyle={{ color:'#f5222d' }} /></Col>
          <Col span={5}><Statistic title="Pending Justifications" value={stats.justificationsPending} prefix={<FileTextOutlined />}     valueStyle={{ color:'#722ed1' }} /></Col>
          <Col span={4}><Statistic title="Total Assigned"         value={stats.total}                 prefix={<AuditOutlined />}        valueStyle={{ color:'#1890ff' }} /></Col>
        </Row>
        ...
        <Tabs activeKey={activeTab} onChange={setActiveTab}>
          <TabPane tab={<Badge count={stats.pending} offset={[10,0]}><span>Pending Approval</span></Badge>} key="pending">
            <Table columns={columns} dataSource={getFilteredRequests()} loading={loading} rowKey="_id"
              pagination={{ pageSize:10, showSizeChanger:true, showTotal:(t,r) => `${r[0]}-${r[1]} of ${t}` }}
              scroll={{ x:1400 }} size="small"
              rowClassName={r => `cash-request-row${canUserApprove(r) ? ' pending-approval-row' : ''}${r.urgency === 'high' ? ' high-urgency-row' : ''}`}
            />
          </TabPane>
          <TabPane tab={<Badge count={stats.approved} offset={[10,0]}><span>Approved</span></Badge>} key="approved">
            <Table columns={columns} dataSource={getFilteredRequests()} loading={loading} rowKey="_id"
              pagination={{ pageSize:10, showTotal:(t,r) => `${r[0]}-${r[1]} of ${t}` }} scroll={{ x:1400 }} size="small" />
          </TabPane>
          <TabPane tab={<Badge count={stats.rejected} offset={[10,0]}><span>Rejected</span></Badge>} key="rejected">
            <Table columns={columns} dataSource={getFilteredRequests()} loading={loading} rowKey="_id"
              pagination={{ pageSize:10, showTotal:(t,r) => `${r[0]}-${r[1]} of ${t}` }} scroll={{ x:1400 }} size="small" />
          </TabPane>
          <TabPane tab={<Badge count={stats.justificationsPending} offset={[10,0]}><span><FileTextOutlined /> Justification Approvals</span></Badge>} key="justifications">
            {activeTab === 'justifications' && (
              <Table columns={justificationColumns} dataSource={justifications} loading={loading} rowKey="_id"
                pagination={{ pageSize:10, showTotal:(t,r) => `${r[0]}-${r[1]} of ${t}` }} scroll={{ x:1200 }} size="small"
      ...
      <Modal
        title={<Space><AuditOutlined />Cash Request Approval</Space>}
        open={approvalModalVisible}
        onCancel={() => { setApprovalModalVisible(false); setSelectedRequest(null); form.resetFields(); }}
        footer={null} width={900}
      >
        {selectedRequest && (
          <div>
            <Alert message="Please review all details before submitting your decision." type="info" showIcon style={{ marginBottom:'16px' }} />

            <Descriptions bordered column={2} size="small" style={{ marginBottom:'20px' }}>
              <Descriptions.Item label="Employee"><Text strong>{selectedRequest.employee?.fullName}</Text></Descriptions.Item>
              <Descriptions.Item label="Department"><Tag color="blue">{selectedRequest.employee?.department}</Tag></Descriptions.Item>
              <Descriptions.Item label="Mode">{getRequestModeTag(selectedRequest.requestMode)}</Descriptions.Item>
              <Descriptions.Item label="Type">{selectedRequest.requestType?.replace(/-/g,' ').replace(/\b\w/g, l=>l.toUpperCase())}</Descriptions.Item>
              ...
              <Descriptions.Item label="Amount">
                {selectedRequest.requestMode === 'reimbursement' ? (
                  <div>
                    <Text>Advance: XAF {(selectedRequest.advanceReceived||0).toLocaleString()}</Text><br />
                    <Text>Spent: XAF {(selectedRequest.amountSpent||0).toLocaleString()}</Text><br />
                    <Text strong>Due: XAF {((selectedRequest.amountSpent||0)-(selectedRequest.advanceReceived||0)).toLocaleString()}</Text>
                  </div>
                ) : (
              ...
              <Descriptions.Item label="Purpose" span={2}>
                <div dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(selectedRequest.purpose||'') }}
                  style={{ whiteSpace:'pre-wrap', wordBreak:'break-word' }} />
              </Descriptions.Item>
              {selectedRequest.businessJustification && (
                <Descriptions.Item label="Business Justification" span={2}>{selectedRequest.businessJustification}</Descriptions.Item>
              )}
            </Descriptions>

            <ItemizedTable data={selectedRequest.itemizedBreakdown} />

            {selectedRequest.requestMode === 'reimbursement' && selectedRequest.reimbursementDetails?.receiptDocuments?.length > 0 && (
              <Card size="small" title={<Space><FileTextOutlined />Receipt Documents</Space>} style={{ marginBottom:'20px' }}>
                <Alert message="Please verify receipts match expenses claimed" type="warning" showIcon style={{ marginBottom:'12px' }} />
                <Space wrap>
                  {selectedRequest.reimbursementDetails.receiptDocuments.map((r,i) => (
                    <Space key={i}>
                      <Button size="small" icon={<EyeOutlined />} onClick={() => handleViewAttachment(r)}>View</Button>
                      <Button size="small" icon={<FileOutlined />} onClick={() => handleDownloadAttachment(r)}>{r.name || `Receipt ${i+1}`}</Button>
                    </Space>
                  ))}
                </Space>
              </Card>
            ...
            {selectedRequest.requestMode === 'advance' && selectedRequest.attachments?.length > 0 && (
              <Card size="small" title="Attached Files" style={{ marginBottom:'20px' }}>
                <Space wrap>
                  {selectedRequest.attachments.map((a,i) => (
                    <Space key={i}>
                      <Button size="small" icon={<EyeOutlined />} onClick={() => handleViewAttachment(a)}>View</Button>
                      <Button size="small" icon={<FileOutlined />} onClick={() => handleDownloadAttachment(a)}>Download {a.name}</Button>
                    </Space>
                  ))}
                </Space>
              </Card>
            ...
            <Card size="small" title="Approval Chain" style={{ marginBottom:'20px' }}>
              <ApprovalTimeline chain={selectedRequest.approvalChain} currentStatus={selectedRequest.status} userEmail={user?.email} />
            </Card>
      ...
      <Modal
        title={<Space><AuditOutlined />Justification Decision</Space>}
        open={justDecisionModalVisible}
        onCancel={() => { setJustDecisionModalVisible(false); setSelectedJustification(null); form.resetFields(); }}
        footer={null} width={900}
      >
        {selectedJustification && (() => {
          const { totalDisbursed } = getDisbursementSummary(selectedJustification);
          const spent    = selectedJustification.justification?.amountSpent    || 0;
          const returned = selectedJustification.justification?.balanceReturned || 0;
          const chainLen = selectedJustification.justificationApprovalChain?.length || 0;
          const chainApproved = selectedJustification.justificationApprovalChain?.filter(s=>s.status==='approved').length || 0;
          return (
            <div>
              <Alert message="Review this justification carefully before making a decision." type="info" showIcon style={{ marginBottom:'16px' }} />

              <Card size="small" style={{ marginBottom:'16px', backgroundColor:'#f0f2f5' }}>
                <Row gutter={16}>
                  <Col span={12}><Text strong>Status:</Text><div style={{ marginTop:'8px' }}>{getStatusTag(selectedJustification.status)}</div></Col>
                  <Col span={12}><Text strong>Approval Progress:</Text>
                    <Progress style={{ marginTop:'8px' }}
                      percent={chainLen ? Math.round((chainApproved/chainLen)*100) : 0}
              ...
              <Descriptions bordered column={2} size="small" style={{ marginBottom:'20px' }}>
                ...
                <Descriptions.Item label="Request ID"><Text code copyable>REQ-{selectedJustification._id?.slice(-6).toUpperCase()}</Text></Descriptions.Item>
                <Descriptions.Item label="Total Disbursed"><Text strong>XAF {totalDisbursed.toLocaleString()}</Text></Descriptions.Item>
              ...
              <Card size="small" title="Financial Summary" style={{ marginBottom:'20px' }}>
                <Row gutter={16}>
                  <Col span={8}><Statistic title="Amount Spent"    value={spent}    prefix="XAF " valueStyle={{ color:'#f5222d' }} /></Col>
                  <Col span={8}><Statistic title="Balance Returned" value={returned} prefix="XAF " valueStyle={{ color:'#52c41a' }} /></Col>
                  <Col span={8}><Statistic title="Total Accounted"  value={spent+returned} prefix="XAF " /></Col>
                </Row>
                {Math.abs((spent+returned) - totalDisbursed) > 1 && (
                  <Alert style={{ marginTop:'12px' }} type="warning" showIcon
                    message={`Discrepancy: disbursed XAF ${totalDisbursed.toLocaleString()} ≠ spent+returned XAF ${(spent+returned).toLocaleString()}`} />
                )}
              </Card>

              <Card size="small" title="Spending Details" style={{ marginBottom:'20px' }}>
                <Paragraph style={{ fontStyle:'italic', backgroundColor:'#f5f5f5', padding:'12px', borderRadius:'4px' }}>
                  {selectedJustification.justification?.details || 'No details provided'}
                </Paragraph>
              </Card>

              <ItemizedTable data={selectedJustification.justification?.itemizedBreakdown || selectedJustification.itemizedBreakdown} />

              {selectedJustification.justificationApprovalChain?.length > 0 && (
                <Card size="small" title="Approval Chain Progress" style={{ marginBottom:'20px' }}>
                  <ApprovalTimeline
                    chain={selectedJustification.justificationApprovalChain}
                    currentStatus={selectedJustification.status}
                    userEmail={user?.email}
                  />
                </Card>
              )}

              {selectedJustification.justification?.documents?.length > 0 && (
                <Card size="small" title={`Supporting Documents (${selectedJustification.justification.documents.length})`} style={{ marginBottom:'20px' }}>
                  <Space wrap>
                    {selectedJustification.justification.documents.map((d,i) => (
                      <Button key={i} icon={<FileOutlined />} size="small" onClick={() => handleDownloadAttachment(d)}>
                        {d.name || `Document ${i+1}`}
                      </Button>
                    ))}
                  </Space>
              ...
              <DecisionForm
                ...
                onCancel={() => { setJustDecisionModalVisible(false); setSelectedJustification(null); form.resetFields(); }}
                submitLabel="Submit Justification Decision"
              />
            </div>
          );
        })()}
      ...
      {/* ── Details Modal ─────────────────────────────────────────────────── */}
      <Modal
        title={<Space><DollarOutlined />{selectedJustification ? 'Justification Details' : 'Cash Request Details'}</Space>}
        open={detailsModalVisible}
        onCancel={() => { setDetailsModalVisible(false); setSelectedRequest(null); setSelectedJustification(null); }}
        footer={null} width={900}
      >
        {/* Cash request details branch */}
        {selectedRequest && !selectedJustification && (
          <div>
            <Descriptions bordered column={2} size="small" style={{ marginBottom:'20px' }}>
              <Descriptions.Item label="Request ID"><Text code copyable>REQ-{selectedRequest._id?.slice(-6).toUpperCase()}</Text></Descriptions.Item>
              <Descriptions.Item label="Status">{getStatusTag(selectedRequest.status, selectedRequest)}</Descriptions.Item>
              <Descriptions.Item label="Employee"><Text strong>{selectedRequest.employee?.fullName}</Text></Descriptions.Item>
              <Descriptions.Item label="Department"><Tag color="blue">{selectedRequest.employee?.department}</Tag></Descriptions.Item>
              <Descriptions.Item label="Mode">{getRequestModeTag(selectedRequest.requestMode)}</Descriptions.Item>
              <Descriptions.Item label="Amount"><Text strong>XAF {(selectedRequest.amountRequested||0).toLocaleString()}</Text></Descriptions.Item>
              <Descriptions.Item label="Type">{selectedRequest.requestType?.replace(/-/g,' ').replace(/\b\w/g, l=>l.toUpperCase())}</Descriptions.Item>
              <Descriptions.Item label="Urgency">{getUrgencyTag(selectedRequest.urgency)}</Descriptions.Item>
              <Descriptions.Item label="Required Date">
                {selectedRequest.requiredDate ? new Date(selectedRequest.requiredDate).toLocaleDateString('en-GB') : 'N/A'}
            ...
            {selectedRequest.attachments?.length > 0 && (
              <Card size="small" title="Attached Files" style={{ marginBottom:'20px' }}>
                <Space wrap>
                  {selectedRequest.attachments.map((a,i) => (
                    <Button key={i} icon={<FileOutlined />} onClick={() => handleDownloadAttachment(a)}>
                      {a.name || a.fileName}
                    </Button>
                  ))}
                </Space>
            ...
            {selectedRequest.approvalChain?.length > 0 && (
              <>
                <Title level={4}><HistoryOutlined /> Approval Chain</Title>
                <ApprovalTimeline chain={selectedRequest.approvalChain} currentStatus={selectedRequest.status} userEmail={user?.email} />
                <Card size="small" title="Current Status" style={{ marginTop:'16px' }}>
                  <Row gutter={16}>
                    <Col span={12}><Text strong>Status: </Text>{getStatusTag(selectedRequest.status, selectedRequest)}</Col>
                    <Col span={12}><Text strong>Progress: </Text>
                      <Progress percent={getApprovalProgress(selectedRequest)} size="small"
                        status={selectedRequest.status === 'denied' ? 'exception' : 'active'} />
                    </Col>
                  </Row>
                  {canUserApprove(selectedRequest) && (
                    <Alert style={{ marginTop:'12px' }} message="Action Required"
                      description="This request is waiting for your approval."
                      type="warning" showIcon
                      action={<Button size="small" type="primary" onClick={() => {
                ...
                {/* Disbursement summary (V2 aware) */}
                {(() => {
                  const ds = getDisbursementSummary(selectedRequest);
                  if (!ds.disbursements.length && !selectedRequest.amountApproved) return null;
                  return (
                    <Card size="small" title="Financial Status" style={{ marginTop:'16px' }}>
                      <Row gutter={16}>
                        {selectedRequest.amountApproved && (
                          <Col span={8}><Statistic title="Amount Approved" value={selectedRequest.amountApproved} prefix="XAF " /></Col>
                        )}
                        <Col span={8}><Statistic title="Total Disbursed" value={ds.totalDisbursed} prefix="XAF " /></Col>
                        {ds.remainingBalance > 0 && (
                          <Col span={8}><Statistic title="Remaining" value={ds.remainingBalance} prefix="XAF " /></Col>
                        )}
                      </Row>
                    </Card>
                  );
                ...
                {selectedRequest.justification && (
                  <Card size="small" title="Justification Details" style={{ marginTop:'16px' }}>
                    <Descriptions column={2} size="small">
                      <Descriptions.Item label="Amount Spent">XAF {selectedRequest.justification.amountSpent?.toLocaleString()}</Descriptions.Item>
                      <Descriptions.Item label="Balance Returned">XAF {selectedRequest.justification.balanceReturned?.toLocaleString()}</Descriptions.Item>
                      <Descriptions.Item label="Details" span={2}>{selectedRequest.justification.details}</Descriptions.Item>
                    </Descriptions>
                    {selectedRequest.justification.documents?.length > 0 && (
                      <Space wrap style={{ marginTop:'8px' }}>
                        {selectedRequest.justification.documents.map((d,i) => (
                          <Button key={i} icon={<FileOutlined />} size="small" onClick={() => handleDownloadAttachment(d)}>
                            {d.name}
                          </Button>
                        ))}
        ...
        {selectedJustification && (() => {
          ...
          return (
            <div>
              <Alert message="Justification Review" type="info" showIcon style={{ marginBottom:'16px' }} />
              <Descriptions bordered column={2} size="small" style={{ marginBottom:'20px' }}>
                <Descriptions.Item label="Employee"><Text strong>{selectedJustification.employee?.fullName}</Text></Descriptions.Item>
                <Descriptions.Item label="Request ID"><Text code copyable>REQ-{selectedJustification._id?.slice(-6).toUpperCase()}</Text></Descriptions.Item>
                <Descriptions.Item label="Total Disbursed"><Text strong>XAF {totalDisbursed.toLocaleString()}</Text></Descriptions.Item>
              ...
              {canUserApproveJustification(selectedJustification) && (
                <Alert style={{ marginTop:'12px' }} message="Action Required"
                  description="This justification is waiting for your decision."
                  type="warning" showIcon
                  action={<Button size="small" type="primary" onClick={() => {
                    setDetailsModalVisible(false);
                    setJustDecisionModalVisible(true);
                  }}>Make Decision</Button>}
                />
              )}
            </div>
      ...
      <Modal
        ...
        footer={[
          <Button key="download" onClick={() => {
            ...
            const a = document.createElement('a');
            a.href = viewingFile.url; a.download = viewingFile.name || 'file';
            document.body.appendChild(a); a.click(); document.body.removeChild(a);
            message.success('Downloaded');
          }}>Download</Button>,
          <Button key="close" onClick={closeFileViewer}>Close</Button>
        ]}
      ...
      >
        {fileViewerLoading ? (
          <div style={{ display:'flex', justifyContent:'center', alignItems:'center', minHeight:'400px' }}>
            <Spin size="large" /><div style={{ marginLeft:'16px' }}>Loading file…</div>
          </div>
        ) : viewingFile ? (
          viewingFile.type === 'image' ? (
            <img src={viewingFile.url} alt={viewingFile.name}
              style={{ maxWidth:'100%', maxHeight:'70vh', objectFit:'contain', border:'1px solid #d9d9d9', borderRadius:'6px' }} />
          ) : viewingFile.type === 'pdf' ? (
            <PDFViewer url={viewingFile.url} name={viewingFile.name} authHeaders={viewingFile.authHeaders} />
          ) : (
            <Alert message="Preview not available"
              description={`"${viewingFile.mimetype}" cannot be previewed. Please download.`}
              type="info" showIcon />
          )
        ) : null}
      </Modal>
      ...
      <style>{`
        ...
        .high-urgency-row { border-left: 3px solid #ff4d4f; }
        .high-urgency-row:hover { background-color: #fff2f0 !important; }
      `}</style>
    </div>
  );
};
