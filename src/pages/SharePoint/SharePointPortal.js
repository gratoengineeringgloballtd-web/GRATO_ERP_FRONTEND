...
const SharePointPortal = () => {
  ...
  const handleNavigateFolder = useCallback((folder) => {
    ...
    setBreadcrumbs(buildBreadcrumbForFolder(folder));
  }, [folders]);

  // Walks parentFolder links within the already-fetched flat folder list to build
  // Home > Grandparent > Parent > This Folder without any extra API calls.
  const buildBreadcrumbForFolder = (folder) => {
    const chain = [];
    let node = folder;
    const visited = new Set();
    while (node && !visited.has(node._id)) {
      visited.add(node._id);
      chain.unshift({ label: node.name, key: node._id });
      const parentId = node.parentFolder?._id || node.parentFolder;
      node = parentId ? folders.find(f => f._id === parentId) : null;
    }
    return [{ label: 'Home', key: 'home' }, ...chain];
  };
  ...
  const handleCreateFolder = async (values) => {
    try {
      ...
      await sharepointAPI.createFolder({
        ...
        ...(user.role === 'admin' && !currentFolder ? { department: values.department } : {}),
        privacyLevel: values.privacyLevel,
        // Creating while a folder is open makes this a subfolder of it; the server
        // inherits department from the parent and enforces privacy can't be looser.
        ...(currentFolder ? { parentFolderId: currentFolder } : {}),
      });
      message.success(currentFolder ? 'Subfolder created successfully' : 'Folder created successfully');
      form.resetFields();
      setFolderModalVisible(false);
      await fetchFolders();
    } catch (error) {
  ...
  // ── Sidebar tree ───────────────────────────────────────────────────────────
  // Recursively builds real nested tree nodes from the flat `folders` list using
  // parentFolder links, instead of a flat per-department leaf list that couldn't
  // represent subfolders at all.
  const buildFolderNode = (folder) => {
    const children = folders
      .filter(f => (f.parentFolder?._id || f.parentFolder) === folder._id)
      .map(buildFolderNode);
  ...
  const byDept = rootFolders.reduce((acc, f) => {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {/* Create Folder Modal */}
      <Modal title={currentFolder ? `New Subfolder in "${currentFolderData?.name}"` : 'Create New Folder'} open={folderModalVisible}
        onOk={() => form.submit()}
        ...
        <Form form={form} layout="vertical" onFinish={handleCreateFolder}>
          ...
          {currentFolder && (
            <Form.Item label="Department">
              <Input value={currentFolderData?.department} disabled style={{ color: '#595959' }} />
              <Text type="secondary" style={{ fontSize: 12 }}>
                Subfolders inherit their parent folder's department.
              </Text>
            </Form.Item>
          )}
          {!currentFolder && user.role === 'admin' && (
            <Form.Item
              name="department"
              label="Department"
