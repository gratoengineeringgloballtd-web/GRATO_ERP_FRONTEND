...
const HRCommunicationsHub = () => {
  ...
  const handleDuplicateCommunication = async (id) => {
    try {
      ...
      navigate('/hr/communications/new', {
        state: {
          template: {
  ...
  const getActionMenu = (record) => (
    <Menu>
      <Menu.Item
        key="view"
        icon={<EyeOutlined />}
        onClick={() => navigate(`/hr/communications/${record._id}`)}
      >
        View Details
      </Menu.Item>
      ...
      {record.status !== 'sent' && (
        <Menu.Item
          key="edit"
          icon={<EditOutlined />}
          onClick={() => navigate(`/hr/communications/${record._id}/edit`)}
        >
          Edit
        </Menu.Item>
      )}
        ...
        <Menu.Item
          ...
          onClick={() => {
            Modal.confirm({
              ...
              onOk: async () => {
                try {
                  await api.post(`/communications/${record._id}/send`);
  ...
  return (
    <div style={{ padding: '24px' }}>
        ...
        <Row align="middle" justify="space-between">
          <Col>
            <Title level={2} style={{ color: 'white', margin: 0 }}>
              <NotificationOutlined /> HR Communications Hub
            </Title>
            <Text style={{ color: 'rgba(255,255,255,0.85)' }}>
              Manage employee communications and announcements
            </Text>
          </Col>
          <Col>
            <Space>
              <Button
                ...
                size="large"
                icon={<PlusOutlined />}
                onClick={() => navigate('/hr/communications/new')}
                style={{ background: 'white', color: '#13c2c2', borderColor: 'white' }}
              >
                New Message
              </Button>
            </Space>
      ...
      {/* HR Access Notice */}
      <Alert
        message="HR Portal Access"
        description="As an HR member, you can create and send communications. You can only view and manage your own messages. Sent messages cannot be deleted."
        type="info"
      ...
      <Card style={{ marginBottom: '24px' }}>
        <Space wrap size="middle">
          <Button
            ...
            onClick={() => navigate('/hr/communications/history')}
          >
            My Messages
          </Button>
          <Button
            icon={<FileTextOutlined />}
            onClick={() => navigate('/hr/communications/templates')}
          >
            Templates
          </Button>
          <Button
            icon={<ClockCircleOutlined />}
            onClick={() => navigate('/hr/communications/scheduled')}
          >
            Scheduled
          </Button>
        </Space>
