...
// ─────────────────────────────────────────────────────────────────────────────
// Resolve the correct download ID from an attachment object.
//
// Priority:
//   1. attachment.url  — if it's a full Cloudinary https URL, use it directly
//   2. attachment.publicId — the stored Cloudinary public_id (may or may not have extension)
//   3. Fallback: extract from URL
//
// ⚠️  DO NOT strip extensions — raw files (PDFs, docs) need their extension
//     in the public_id for Cloudinary to find them.
// ─────────────────────────────────────────────────────────────────────────────
const resolveDownloadId = (attachment) => {
  if (!attachment) return null;

  // If url is a full Cloudinary URL, use it directly — no API lookup needed
  if (attachment.url && (
    attachment.url.startsWith('https://res.cloudinary.com') ||
    attachment.url.startsWith('http://res.cloudinary.com')
  )) {
    return attachment.url; // fileRoutes /download will redirect straight to this
  }

  // Use publicId as-is — preserve extension for raw (PDF/doc) files
  if (attachment.publicId) {
    return attachment.publicId;
  }

  // Last resort: extract path from URL (without stripping extension)
  if (attachment.url) {
    const urlParts = attachment.url.split('/');
    const uploadIndex = urlParts.findIndex(part => part === 'upload');
    if (uploadIndex !== -1 && uploadIndex + 2 < urlParts.length) {
      // Skip the version segment (vXXXXXX) if present
      const afterUpload = urlParts.slice(uploadIndex + 1);
      const versionSkip = afterUpload[0]?.match(/^v\d+$/) ? 1 : 0;
      return afterUpload.slice(versionSkip).join('/');
      // ✅ NO extension stripping here
    }
  }
...
const RequestDetails = () => {
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // handleDownloadAttachment — fixed
  // ─────────────────────────────────────────────────────────────────────────────
  const handleDownloadAttachment = async (attachment) => {
    ...
    try {
      ...
      const downloadId = resolveDownloadId(attachment);

      if (!downloadId) {
        console.error('Could not resolve download ID from attachment:', attachment);
      ...
      // Always encodeURIComponent — the backend /download/:publicId(*) route decodes it.
      // Full https:// URLs are also encoded here; the backend detects and proxies them.
  ...
  const handleViewAttachment = async (attachment) => {
    try {
      ...
      if (!downloadId) {
        console.error('Could not resolve download ID:', attachment);
        message.error('Unable to locate file. Invalid attachment data.');
        return;
      }
      ...
      // Always route through the API — the server proxies raw files (PDFs, docs)
      // so the browser never hits Cloudinary directly (which 401s for raw resources).
      // The backend's /download route already handles full https:// URLs passed as the id.
      const apiBaseUrl = process.env.REACT_APP_API_URL || 'http://localhost:5001/api';
      const encodedId  = encodeURIComponent(downloadId);

      // ── PDF: use PDFViewer — it fetches via /files/view/ with auth ─────────
      if (isPDF) {
        setViewingFile({
          name:     attachment.name,
          // Pass the full encoded API view URL; PDFViewer will fetch it with the token
          publicId: downloadId,
          type:     'pdf',
      ...
      // ── Image / other: fetch blob via API and create object URL ───────────
      const downloadUrl = `${apiBaseUrl}/files/download/${encodedId}`;
        ...
        {fileViewerLoading ? (
          ...
          <div>
            {viewingFile.type === 'image' ? (
              ...
              <PDFViewer 
                publicId={viewingFile.publicId || viewingFile.url}
...
//       if (downloadId.startsWith('https://') || downloadId.startsWith('http://')) {
//         setViewingFile({
//           name:     attachment.name,
//           url:      downloadId,   // direct Cloudinary URL
...
//       // ── PDF: use PDFViewer component (it fetches via /files/view/) ─────────
//       if (isPDF) {
//         setViewingFile({
//           name:     attachment.name,
//           publicId: downloadId,
...
//       // ── Image / other: fetch blob and create object URL ────────────────────
//       const apiBaseUrl  = process.env.REACT_APP_API_URL || 'http://localhost:5001/api';
//       const downloadUrl = `${apiBaseUrl}/files/download/${encodeURIComponent(downloadId)}`;
