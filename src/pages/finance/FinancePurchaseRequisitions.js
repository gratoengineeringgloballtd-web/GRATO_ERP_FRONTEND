...
const FinancePurchaseRequisitions = () => {
    ...
    const fetchFinanceJustifications = async () => {
        try {
            const response = await makeAuthenticatedRequest(`${API_BASE_URL}/purchase-requisitions/finance`);
            if (response.success && response.data) {
                setJustifications(
                    response.data.filter(r => r.status === 'justification_pending_finance')
                );
            }
        } catch (error) {
            console.error('Error fetching finance justifications:', error);
        }
    };

    const handleFinanceJustificationDecision = async () => {
        ...
        try {
            ...
            const response = await makeAuthenticatedRequest(
                `${API_BASE_URL}/purchase-requisitions/${selectedJust._id}/justification-decision`,
                {
                    method: 'POST',
                    body: JSON.stringify({ decision: justDecision, comments: justComments })
                }
            );
            if (response.success) {
        ...
        <div style={{ padding: '24px' }}>
            ...
            {mainSection === 'requisitions' && (
                <Card>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
                        ...
                        <Space>
                            <Dropdown
                                menu={{
                                    items: [
                                        { key: 'requisition_summary', label: 'Summary Report (all requisitions)' },
                                        { key: 'requisition_spend', label: 'Spend Report (approved vs disbursed)' },
                                        { key: 'requisition_pending_approvals', label: 'Pending Approvals Digest' }
                                    ],
                                    onClick: async ({ key }) => {
                                        const result = await purchaseRequisitionAPI.exportRequisitionReport(key);
                                        if (result.success) {
                                            message.success('Report download started');
                                        } else {
                                            message.error(result.message || 'Failed to export report');
                                        }
                                    }
                                }}
                            >
                    ...
                    <Tabs activeKey={activeTab} onChange={handleTabChange} style={{ marginBottom: '16px' }}>
                        ...
                        >
                            {justifications.length === 0 ? (
                                <Alert message="No justifications pending your finance review." type="info" showIcon />
                            ) : (
                                <Table
                                    ...
                                    columns={[
                                        ...
                                        },
                                        {
                                            ...
                                            render: (_, r) => (
                                                <div>
                                                    <Text>Budget: XAF {(r.budgetXAF || 0).toLocaleString()}</Text><br />
                                                    <Text type="secondary" style={{ fontSize: '11px' }}>
                                                        Disbursed: XAF {(r.totalDisbursed || 0).toLocaleString()}
                                                    </Text>
                                                </div>
                                            ),
                                            width: 160
                                            ...
                                            render: (_, r) => (
                                                <Space>
                                                    <Button size="small" type="primary" icon={<AuditOutlined />}
                                                        onClick={async () => {
                                                            try {
                                                                const res = await makeAuthenticatedRequest(`${API_BASE_URL}/purchase-requisitions/${r._id}`);
                                                                if (res.success) { setSelectedJust(res.data); setJustDrawerVisible(true); }
                    ...
                    {/* ── Search & Filter Bar (only shown on non-justification tabs) ── */}
            ...
            {/* ── Finance Justification Review Drawer ──────────────────────── */}
            <Drawer
                title={<Space><BankOutlined />Finance Justification Review</Space>}
                placement="right"
                width={860}
                    ...
                    return (
                        <div>
                            <Descriptions bordered column={2} size="small" style={{ marginBottom: '16px' }}>
                                ...
                                <Descriptions.Item label="Disbursed"><Text strong style={{ color: '#52c41a' }}>XAF {totalDisbursed.toLocaleString()}</Text></Descriptions.Item>
                            </Descriptions>
                            ...
                            <Card size="small" title="Financial Reconciliation" style={{ marginBottom: '16px' }}>
                                ...
                                <div style={{ marginTop: '12px' }}>
                                    {balanced
                                        ? <Alert message="Reconciled correctly" type="success" showIcon />
                                        : <Alert message={`Discrepancy of XAF ${Math.abs((spent + returned) - totalDisbursed).toLocaleString()}`} type="warning" showIcon />
                                    }
                                </div>
                            </Card>

                            {j.actualExpenses?.length > 0 && (
                                <Card size="small" title="Expense Items" style={{ marginBottom: '16px' }}>
                                    <Table
                                        ...
                                        columns={[
                                            ...
                                            { title: 'Category', dataIndex: 'category', key: 'cat', render: c => <Tag>{c}</Tag>, width: 120 },
                            ...
                            {j.receipts?.length > 0 && (
                                <Card size="small" title={`Receipts (${j.receipts.length})`} style={{ marginBottom: '16px' }}>
                                    <Space wrap>
                                        {j.receipts.map((r, i) => (
                                            <Button key={i} size="small" icon={<FilePdfOutlined />}
                            ...
                            <Card size="small" title={<Space><BankOutlined />Finance Decision</Space>}
                                style={{ borderColor: '#722ed1' }} headStyle={{ backgroundColor: '#f9f0ff' }}>
                                <Alert message="Verify the financial reconciliation is accurate before deciding." type="info" showIcon style={{ marginBottom: '16px' }} />
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                                    ...
                                    <div>
                                        ...
                                        <TextArea rows={3} placeholder="Finance comments…" value={justComments} onChange={e => setJustComments(e.target.value)} showCount maxLength={500} />
                                    </div>
                                    <Space>
                                        <Button type="primary" loading={justActionLoading} icon={<SendOutlined />} onClick={handleFinanceJustificationDecision}
