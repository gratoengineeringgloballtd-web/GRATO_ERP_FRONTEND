...
import { ITDispatchForm, ITDispatchStatusCard } from './ITDispatchSection';
...
const ITRequestDetails = () => {
  ...
  useEffect(() => {
    if (request && user) {
      ...
      console.log('User:', { email: user.email, role: user.role, department: user.department });
      console.log('Request:', { ticketNumber: request.ticketNumber, status: request.status });
  ...
  const getStatusTag = (status) => {
    const statusMap = {
      ...
      'pending_discharge': { color: 'orange', icon: <ClockCircleOutlined />, text: 'Pending Dispatch' },
      'pending_acknowledgment': { color: 'gold', icon: <ClockCircleOutlined />, text: 'Awaiting Acknowledgment' },
  ...
  const isPendingIT = () =>
    request?.status === 'pending_it_approval' || request?.status === 'pending_it_review';

  // ─── Dispatch-related status helpers ───
  const isItUser = user?.role === 'it' || user?.role === 'admin';

  // Show dispatch form: material request + resolved + IT user + not yet dispatched
  const showDispatchForm =
    isItUser &&
    request?.requestType === 'material_request' &&
    request?.status === 'resolved' &&
    !request?.dischargedItems?.length;

  // Show dispatch status card: dispatched or acknowledged
  ...
  return (
    <div style={{ padding: '24px' }}>
      ...
      {/* Dispatch action alert for IT */}
      {showDispatchForm && (
        <Alert
          message="Dispatch Required"
          description="This material request has been resolved. Please dispatch the items to the employee below."
          type="warning"
          showIcon
          style={{ marginBottom: '24px' }}
        />
      ...
      <Row gutter={[16, 16]}>
        ...
        <Col xs={24} lg={16}>
          ...
          {/* ── DISPATCH FORM (IT only, status=resolved, material request) ── */}
          {showDispatchForm && (
            <ITDispatchForm
              request={request}
              currentUser={user}
          ...
          {/* ── DISPATCH STATUS CARD (IT, after dispatch) ── */}
          {showDispatchStatus && (
            <ITDispatchStatusCard
              request={request}
              currentUser={user}
            />
          )}
        ...
        <Col xs={24} lg={8}>
          ...
          <Card title="Activity Timeline">
            <Timeline>
              ...
              {request.status === 'pending_acknowledgment' && request.dischargeSignature?.signedAt && (
                <Timeline.Item color="orange" dot={<ClockCircleOutlined />}>
                  <Text strong>Items Dispatched</Text>
                  <br />
                  <Text type="secondary" style={{ fontSize: '12px' }}>
                    By {request.dischargeSignature.name} • {dayjs(request.dischargeSignature.signedAt).format('MMM DD, YYYY HH:mm')}
                  </Text>
                  <br />
                  <Text type="secondary" style={{ fontSize: '11px' }}>Awaiting employee acknowledgment</Text>
                </Timeline.Item>
              )}
              ...
              {request.status === 'discharge_complete' && (
                <>
                  ...
                  <Timeline.Item color="green" dot={<CheckCircleOutlined />}>
                    <Text strong>Employee Acknowledged Receipt</Text>
                    <br />
                    <Text type="secondary" style={{ fontSize: '12px' }}>
                      {request.acknowledgmentSignature?.name} • {request.acknowledgmentSignature?.signedAt ? dayjs(request.acknowledgmentSignature.signedAt).format('MMM DD, YYYY HH:mm') : '—'}
                    </Text>
                  </Timeline.Item>
                </>
              )}
      ...
      <Modal
        ...
        destroyOnClose
      >
        <Form form={form} layout="vertical" onFinish={handleITDecision}>
          <Alert
          ...
          <Form.Item name="decision" hidden><Input /></Form.Item>

          <Form.Item name="estimatedCost" label="Estimated Cost (XAF)">
            <Input type="number" min={0} placeholder="Enter estimated cost" />
          </Form.Item>

          <Form.Item name="estimatedCompletionTime" label="Estimated Completion Time">
          ...
          <Form.Item>
            <Space>
              <Button type="primary" htmlType="submit" loading={submitting} disabled={!selectedDecision}>
                Submit Decision
              </Button>
              <Button onClick={() => {
                setModalVisible(false);
