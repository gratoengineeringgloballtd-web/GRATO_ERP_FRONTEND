...
import { itInventoryAPI } from '../../services/api';
...
const getConditionTag = (condition) => {
  ...
  const info = map[condition] || { color: 'default', text: condition };
...
const getCategoryIcon = (category) => {
  ...
  return icons[category] || <InboxOutlined style={{ color: '#666' }} />;
};
...
const isLowStock = (item) =>
  item.stockInfo && item.stockInfo.quantity <= item.stockInfo.minStockLevel;
...
const ITInventoryManagement = () => {
  ...
  const fetchInventory = useCallback(async (page = 1, pageSize = 10) => {
    try {
      ...
      if (activeTab === 'hardware')  params.category = 'hardware';
      if (activeTab === 'software')  params.category = 'software';
      if (activeTab === 'assigned')  params.status   = 'assigned';
      if (activeTab === 'low_stock') params.lowStock  = 'true';

      const [inventoryRes, statsRes, locationsRes] = await Promise.all([
        itInventoryAPI.getInventory(params),
        itInventoryAPI.getStats(),
        itInventoryAPI.getLocations(),
      ]);

      setInventory(inventoryRes.data || []);
      setPagination({
        current:  inventoryRes.pagination?.current  || page,
        pageSize,
        total:    inventoryRes.pagination?.totalRecords || 0,
      });
  ...
  const openEditModal = (item) => {
    ...
    form.setFieldsValue({
      ...
      reorderPoint:   item.stockInfo?.reorderPoint,
      notes:          item.notes,
    });
    setModalVisible(true);
  };
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      // Re-nest flat form values into the schema shape
      const payload = {
      ...
      if (modalType === 'add') {
        await itInventoryAPI.create(payload);
        message.success('Inventory item added successfully');
      } else {
        await itInventoryAPI.update(selectedItem._id, payload);
  ...
  const handleDelete = (item) => {
    Modal.confirm({
      ...
      content: `Are you sure you want to delete "${item.itemName}"? This cannot be undone.`,
      okText:  'Delete',
      okType:  'danger',
      onOk: async () => {
        try {
          await itInventoryAPI.delete(item._id);
  ...
  const columns = [
    {
      ...
      render: (code, record) => (
        <div>
          ...
          {isLowStock(record) && (
            <div><Badge status="warning" text="Low Stock" /></div>
          )}
        </div>
      ),
    },
      ...
      render: (_, record) => (
        <div>
          <Space style={{ marginBottom: 4 }}>
            ...
            <Text strong style={{ fontSize: 13 }}>{record.itemName}</Text>
          </Space>
          <br />
          ...
          {record.serialNumber && (
            <>
              <br />
              <Text type="secondary" style={{ fontSize: 10 }}>S/N: {record.serialNumber}</Text>
            </>
          )}
        </div>
      ),
    },
    {
      ...
      render: (_, record) => (
        <div>
          <Tag color="blue" style={{ fontSize: 11 }}>{record.category?.toUpperCase()}</Tag>
          <br />
          <Text type="secondary" style={{ fontSize: 10 }}>{record.subcategory}</Text>
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <div>
          {record.stockInfo && (
            <>
              <Text style={{ fontSize: 12 }}>Qty: {record.stockInfo.quantity}</Text>
              <br />
              <Text type="secondary" style={{ fontSize: 10 }}>
                Min: {record.stockInfo.minStockLevel}
              </Text>
              <br />
              {isLowStock(record) && <Tag color="red" size="small">Low Stock</Tag>}
            </>
          )}
        </div>
      ),
        ...
        <div>
          {record.purchaseInfo && (
            <>
              ...
              <br />
              <Text type="secondary" style={{ fontSize: 10 }}>
                {record.purchaseInfo.purchaseDate
              ...
              <Text type="secondary" style={{ fontSize: 10 }}>
                Warranty:{' '}
                {record.purchaseInfo.warrantyExpiry
                  ? new Date(record.purchaseInfo.warrantyExpiry).toLocaleDateString()
                  : '—'}
              </Text>
            </>
          )}
  ...
  const StatsCards = () => {
    ...
    return (
      <Row gutter={16} style={{ marginBottom: 24 }}>
        {[
          { title: 'Total Items',  value: s.totalItems,  color: '#1890ff', icon: <InboxOutlined /> },
          { title: 'Available',    value: s.available,   color: '#52c41a', icon: <CheckCircleOutlined /> },
          { title: 'Assigned',     value: s.assigned,    color: '#1890ff', icon: <InfoCircleOutlined /> },
          { title: 'Maintenance',  value: s.maintenance, color: '#faad14', icon: <WarningOutlined /> },
          { title: 'Low Stock',    value: s.lowStock,    color: s.lowStock > 0 ? '#ff4d4f' : '#52c41a', icon: <WarningOutlined /> },
          {
            title: 'Total Value',
            value: `${((s.totalValue || 0) / 1_000_000).toFixed(1)}M`,
            color: '#722ed1',
            icon: <BarChartOutlined />,
            suffix: 'XAF',
          },
        ].map(({ title, value, color, icon, suffix }) => (
          <Col xs={12} sm={8} md={4} key={title}>
            <Card size="small">
              <Statistic title={title} value={value} valueStyle={{ color }}
                prefix={icon} suffix={suffix} />
            </Card>
          </Col>
        ))}
      </Row>
  ...
  const ViewBody = ({ item }) => (
    <div>
      <Row gutter={16}>
        <Col span={12}>
          <Card title="Basic Information" size="small">
            ...
            <p><strong>Status:</strong> {getStatusTag(item.status)}</p>
            <p><strong>Condition:</strong> {getConditionTag(item.condition)}</p>
          </Card>
        </Col>
        <Col span={12}>
          <Card title="Location & Assignment" size="small">
            ...
            {item.assignedTo && (
              <>
                <p><strong>Assigned To:</strong> {item.assignedTo.fullName}</p>
                <p><strong>Department:</strong> {item.assignedTo.department}</p>
                <p><strong>Employee ID:</strong> {item.assignedTo.employeeId}</p>
              </>
            )}
            {item.assignedDate && (
              <p><strong>Assigned Date:</strong> {new Date(item.assignedDate).toLocaleDateString()}</p>
            )}
          </Card>
        </Col>
      </Row>
      ...
      <Row gutter={16} style={{ marginTop: 16 }}>
          ...
          <Card title="Purchase Information" size="small">
            <p><strong>Purchase Date:</strong>{' '}
              {item.purchaseInfo?.purchaseDate
                ? new Date(item.purchaseInfo.purchaseDate).toLocaleDateString() : '—'}
            </p>
            <p><strong>Purchase Price:</strong> XAF {item.purchaseInfo?.purchasePrice?.toLocaleString() || '—'}</p>
            <p><strong>Supplier:</strong> {item.purchaseInfo?.supplier || '—'}</p>
            <p><strong>Warranty Expiry:</strong>{' '}
              {item.purchaseInfo?.warrantyExpiry
                ? new Date(item.purchaseInfo.warrantyExpiry).toLocaleDateString() : '—'}
            </p>
            <p><strong>Invoice Number:</strong> {item.purchaseInfo?.invoiceNumber || '—'}</p>
            {item.warrantyStatus && (
              <p><strong>Warranty Status:</strong>{' '}
                <Tag color={item.warrantyStatus === 'valid' ? 'green' : item.warrantyStatus === 'expiring_soon' ? 'orange' : 'red'}>
                  {item.warrantyStatus.replace('_', ' ').toUpperCase()}
                </Tag>
              </p>
            )}
          </Card>
        ...
        <Col span={12}>
          <Card title="Stock Information" size="small">
            <p><strong>Current Quantity:</strong> {item.stockInfo?.quantity}</p>
            <p><strong>Minimum Level:</strong> {item.stockInfo?.minStockLevel}</p>
            <p><strong>Maximum Level:</strong> {item.stockInfo?.maxStockLevel}</p>
            <p><strong>Reorder Point:</strong> {item.stockInfo?.reorderPoint}</p>
            {item.isLowStock && <Alert message="Low Stock Alert" type="warning" showIcon />}
          </Card>
        </Col>
      </Row>

      {item.specifications && item.specifications.size > 0 && (
        <Card title="Specifications" size="small" style={{ marginTop: 16 }}>
          {[...item.specifications.entries()].map(([key, val]) => (
            <p key={key}>
              <strong>
      ...
      {item.maintenanceHistory?.length > 0 && (
        <Card title="Maintenance History" size="small" style={{ marginTop: 16 }}>
          {item.maintenanceHistory.map((m, i) => (
            <div key={i} style={{ marginBottom: 8, padding: 8, backgroundColor: '#f9f9f9', borderRadius: 4 }}>
              <p style={{ margin: 0 }}><strong>Date:</strong> {new Date(m.date).toLocaleDateString()}</p>
              <p style={{ margin: 0 }}><strong>Type:</strong> {m.type}</p>
              <p style={{ margin: 0 }}><strong>Description:</strong> {m.description}</p>
              <p style={{ margin: 0 }}><strong>Technician:</strong> {m.technician}</p>
              {m.cost > 0 && <p style={{ margin: 0 }}><strong>Cost:</strong> XAF {m.cost.toLocaleString()}</p>}
            </div>
          ))}
        </Card>
      )}
      ...
      <Row gutter={16}>
        ...
        <Col span={6}>
          <Form.Item name="reorderPoint" label="Reorder Point">
            <InputNumber style={{ width: '100%' }} min={0} placeholder="Reorder" />
          </Form.Item>
        </Col>
      </Row>

      <Form.Item name="notes" label="Notes">
        <TextArea rows={3} placeholder="Enter any additional notes…" showCount maxLength={500} />
      </Form.Item>
    </Form>
  );
  ...
  // Tab counts come from stats (fast, single query) so tabs don't require an
  // extra round-trip when switching.
  const tabCounts = {
    all:       stats?.totalItems || inventory.length,
    hardware:  stats?.byCategory?.hardware    || 0,
    software:  stats?.byCategory?.software    || 0,
  ...
  return (
    <div style={{ padding: 24 }}>
      <Card>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          ...
          <Space>
            <Button icon={<ReloadOutlined />} loading={loading}
              onClick={() => fetchInventory(pagination.current, pagination.pageSize)}>
              Refresh
            </Button>
        ...
        {error && (
          <Alert message="Error Loading Data" description={error} type="error" showIcon closable
        ...
        <Tabs activeKey={activeTab} onChange={(key) => { setActiveTab(key); }}
          ...
          <TabPane tab={<span><InboxOutlined /> All Items ({tabCounts.all})</span>}    key="all" />
          <TabPane tab={<span><DesktopOutlined /> Hardware ({tabCounts.hardware})</span>} key="hardware" />
          <TabPane tab={<span><ToolOutlined /> Software ({tabCounts.software})</span>}    key="software" />
          <TabPane tab={<span><InfoCircleOutlined /> Assigned ({tabCounts.assigned})</span>} key="assigned" />
          <TabPane tab={<span><WarningOutlined /> Low Stock ({tabCounts.low_stock})</span>} key="low_stock" />
        </Tabs>
        ...
        <Card size="small" style={{ marginBottom: 16 }}>
          <Row gutter={16} align="middle">
            <Col flex="auto">
              <Input
                placeholder="Search by code, name, brand, model, serial…"
            ...
            <Col>
              <Select style={{ width: 130 }} value={filters.category}
            ...
            <Col>
              <Select style={{ width: 120 }} value={filters.status}
            ...
            <Col>
              <Select style={{ width: 160 }} value={filters.location}
                onChange={v => setFilters(f => ({ ...f, location: v }))}
                showSearch placeholder="Location">
            ...
            <Col>
              <Button icon={<FilterOutlined />} onClick={() => setFilters({
                category: 'all', status: 'all', location: 'all',
                dateRange: null, searchText: '', lowStock: false
              })}>
                Clear
              </Button>
            </Col>
        ...
        {(stats?.lowStock || 0) > 0 && (
          <Alert
            ...
            description={`${stats.lowStock} item${stats.lowStock !== 1 ? 's are' : ' is'} running low on stock. Consider reordering soon.`}
            type="warning" showIcon style={{ marginBottom: 16 }}
            action={
              <Button size="small" type="primary" onClick={() => setActiveTab('low_stock')}>
        ...
        {inventory.length === 0 && !loading ? (
          <Alert
            ...
            description="No items match your current filters, or the inventory is empty. Click 'Add Item' to get started."
          ...
          <>
            ...
            <Table
              ...
              pagination={{
                ...
                onChange: (page, pageSize) => fetchInventory(page, pageSize),
              }}
      ...
      >
        {modalType === 'view' && selectedItem
          ? <ViewBody item={selectedItem} />
          : <ItemForm />
        }
      </Modal>

      <style>{`
        .low-stock-row  { background-color: #fff2e8 !important; border-left: 3px solid #fa8c16 !important; }
        .low-stock-row:hover { background-color: #ffe7d6 !important; }
        .maintenance-row { background-color: #fff1f0 !important; border-left: 3px solid #ff7a45 !important; }
        .maintenance-row:hover { background-color: #ffe7e6 !important; }
      `}</style>
    </div>
  );
};
